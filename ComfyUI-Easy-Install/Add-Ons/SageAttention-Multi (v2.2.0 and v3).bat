...
cd %DIR_LVL%
if exist ".\Add-Ons\Tools\Helper-CEI\ComfyUI-EZi-Desktop.ico" if exist ".\ComfyUI-EZi SageAttention.bat" (
	echo %green%:::::::::::::: Creating EZi SageAttention Desktop shortcut%reset%
	powershell -NoProfile -ExecutionPolicy Bypass -command "$s=(New-Object -ComObject WScript.Shell).CreateShortcut('%DESKTOP%\ComfyUI-EZi SA.lnk'); $s.TargetPath='%cd%\ComfyUI-EZi SageAttention.bat'; $s.WorkingDirectory='%cd%\'; $s.IconLocation='%cd%\Add-Ons\Tools\Helper-CEI\ComfyUI-EZi-Desktop.ico'; $s.Save();"
)
